...
double distanceCoord(double lat1, double long1, double lat2, double long2) {
    ...
    return 6371 * c; // Rayon de la Terre en kilomètres
}
...
int main() {
    ...
    std::vector<std::vector<Arete>> graphe(villes.size());

    // Création des arêtes (distances euclidiennes)
    for (size_t i = 0; i < villes.size(); ++i) {
        for (size_t j = 0; j < villes.size(); ++j) {
            if (i != j) {
                double distance = distanceCoord(villes[i].latitude, villes[i].longitude, villes[j].latitude, villes[j].longitude);
                graphe[i].push_back({static_cast<int>(j), distance});
            }
        }
    }
    ...
    for (size_t i = 0; i < graphe.size(); ++i) {
        std::cout << villes[i].nom << " -> ";
        for (const auto& arete : graphe[i]) {
            std::cout << villes[arete.destination].nom << "(" << arete.distance << "km) ";
        }
